// Logic Conchord v.0.9
/* ==== CHANGE LOG ====
 * v0.9
 * - NYTT: Voice Lead — kryddtangent (sist i zonen). Medan den hålls (eller
 *         latchas) läggs varje NYTT ackord närmast det föregående: motorn väljer
 *         automatiskt den oktav/inversions-omläggning (samma tonklasser, samma
 *         ackordtyp) som rör tonerna minst. Gemensamma toner hamnar då stilla och
 *         retriggas inte (referensräknad notsändning) -> mjukt, "filmiskt" legato.
 *         Verkar på ackordBYTEN: trycker man den medan ett ackord redan ligger
 *         händer inget förrän nästa ackord. Ankaret = senast byggda ackordet och
 *         uppdateras alltid (även när Voice Lead är av), så gesten kan kopplas in
 *         mitt i en följd och utgår från där du är. Manuell Inversion + hjul/bend
 *         -> Inversion åsidosätts medan ett ackord är voice-lett (motorn väljer
 *         inversionen själv). Bäst ihop med Single Chord Mode.
 * - NYTT: Out-of-Scale Keys får läget "Diminished" — en tangent utanför skalan
 *         (typ en svart tangent i en vit skala) bygger ett kromatiskt dim-ackord
 *         på den tonen i st f att mutas/snappas. Klassiskt filmiskt genomgångs-
...
// Voice Lead: senast byggda ackordets tonhöjder. Nästa ackord läggs närmast detta.
// Uppdateras ALLTID när ett ackord byggs (även när Voice Lead är av), så gesten
// kan kopplas in mitt i en följd och utgå från där du befinner dig. Nollas i Reset.
var voiceLeadAnchor = null;
...
var ZONE_MODIFIERS = [
  ...
  {
    name: "Voice Lead", // nära-ackord: nästa ackord läggs närmast det föregående
    apply: function (s, vel) {
      s.voiceLead = true;
    },
  },
...
function startChord(inputPitch, velocity, channel) {
  var s = getSettings();
  // Voice Lead: lägg detta ackord närmast det föregående (ankaret). Ankaret
  // fryses på recordet så live-morf (reglage/modifiers) inte flyttar målet.
  var anchor = s.voiceLead ? voiceLeadAnchor : null;
  // nytt ackord ärver hållna join-extras (melodin re-harmoniseras till ackordet)
  var built = buildChordNotes(inputPitch, velocity, s, currentJoinPitches(), anchor);
  ...
  // ankaret för NÄSTA ackord = detta ackords toner. Uppdateras alltid (även när
  // Voice Lead är av) så gesten kan kopplas in mitt i en följd och utgå härifrån.
  voiceLeadAnchor = pitchesOf(built);
...
function getSettings() {
  ...
  s.voiceLead = false; // sätts av Voice Lead-modifiern: lägg ackordet närmast förra
...
function buildChordNotes(inputPitch, velocity, s, extras, vlAnchor) {
  ...
  if (degreeInfo.degree === -1) {
    ...
    if (s.outOfScale === 1) return [{ pitch: inputPitch, velocity: velocity }]; // Pass Through
    if (s.outOfScale === 3) {
      // Diminished: en ton utanför skalan blir ett kromatiskt dim-ackord på den
      // tonen. s.dim staplar små terser (+3 per ackordton) oberoende av skalgrad,
  ...
  // Voice Lead åsidosätter manuell/perf-inversion: motorn söker själv den
  // omläggning som ligger närmast föregående ackord. Utan ankare (första ackordet
  // i en gest) faller vi tillbaka på vanlig inversion.
...
// ===== VOICE LEADING (nära ackord) =====

// Väljer den oktav/inversions-omläggning av chord (redan voicad) som rör sig
// minst från anchor. Bevarar tonklassuppsättningen -> samma ackordtyp, bara
// omlagd. Söker hela-ackord-oktavskift x inversionsrotationer och tar lägst
// kostnad. anchor förutsätts vara föregående ackords tonhöjder.
function applyVoiceLeading(chord, anchor) {
  var n = chord.length;
  var shifts = [-24, -12, 0, 12, 24];
  var best = null;
  var bestCost = Infinity;
  for (var si = 0; si < shifts.length; si++) {
    var shifted = [];
    for (var k = 0; k < n; k++) shifted.push(chord[k] + shifts[si]);
    // alla inversioner (rotationer) av det oktavskiftade ackordet
    for (var inv = -(n - 1); inv <= n - 1; inv++) {
      var cand = applyInversion(shifted, inv);
      var cost = voiceLeadCost(cand, anchor);
      if (cost < bestCost) {
        bestCost = cost;
        best = cand;
      }
...
// Lägre = mindre total rörelse. Lika många toner: para ihop sorterat — då hamnar
// gemensamma toner på avstånd 0 (ligger still, retriggas inte). Olika antal:
// varje ny ton till närmaste ankarton.
function voiceLeadCost(cand, anchor) {
  var a = cand.slice(0).sort(function (x, y) {
    return x - y;
  });
  var b = anchor.slice(0).sort(function (x, y) {
    return x - y;
  });
  var cost = 0;
  if (a.length === b.length) {
    for (var i = 0; i < a.length; i++) cost += Math.abs(a[i] - b[i]);
    return cost;
  }
...
// Plocka ut bara tonhöjderna ur ett byggt ackord ([{pitch,velocity}] -> [pitch]).
function pitchesOf(notes) {
  var p = [];
  for (var i = 0; i < notes.length; i++) p.push(notes[i].pitch);
  return p;
}
