...
function HandleMIDI(event) {
  ...
  if (event instanceof PitchBend) {
    ...
    if (GetParameter("Pitch Bend Latch") > 0) {
      ...
      if (raw === 0) return;
      var newGesture =
        pitchBendValue === 0 || raw > 0 !== pitchBendValue > 0;
...
function buildChordNotes(inputPitch, velocity, s) {
  ...
  // perf-offset (hjul/bend) hoppar över 1-notsackord — där blir "inversion"
  // bara oktavtransponering. UI-slidern appliceras alltid.
